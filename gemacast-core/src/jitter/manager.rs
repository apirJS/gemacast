...
use super::target::{TargetBreakdown, TargetController, TargetTerm};
...
const LOG_INTERVAL_FRAMES: u32 = 1000 / MILLIS_PER_FRAME;
...
impl JitterBufferManager {
    pub fn new(
        ...
        Self {
            ...
            log_frame_count: 0,
            log_window_started_at: None,
            frames_played: 0,
            tally: TimescaleTally::default(),
        }
    }
    ...
    fn note_splice_quality(&mut self, rms: f32) {
        self.tally.splice_rms_sum += rms;
        self.tally.splice_rms_count += 1;
        if let Some(step) = self.timescale.take_splice_step() {
            self.tally.splice_step_max = self.tally.splice_step_max.max(step);
        }
    }
        ...
        while let Some(pkt) = consumer.try_pop() {
            ...
            match self.buffer.insert(pkt) {
                ...
                InsertResult::Stale => {
                    ...
                    self.tally.stale_rejects += 1;
                    self.tally.stale_lag_sum += lag as u64;
                    self.tally.stale_lag_max = self.tally.stale_lag_max.max(lag);
                }
    ...
    fn process_next_frame(&mut self) {
        ...
        if self.buffer.occupied_count() > 0 && !self.buffer.has_next() {
            ...
            if let Some(lo) = self.buffer.lowest_available_seq() {
                ...
                self.tally.playhead_skips += 1;
                self.tally.skipped_frames += 1;
            ...
            if let Some(lo) = fast_forward_seq {
                ...
                self.tally.playhead_skips += 1;
                self.tally.skipped_frames += diff as u32;
                if diff > 20 {
                    self.decoder.resync();
                }
                self.flow.gap_hold_count = 0;
        ...
        if self.buffer.has_next() {
            ...
            self.observe_delivery(false, now);
            ...
            if self.flow.starvation_count > 0 {
                if !tcp_mode {
                    ...
                    if self.flow.starvation_recovery == 0 {
                        self.control.record_starvation(now);
                        self.control
                    ...
                    self.flow.arm_recovery_window(now);
                }
            ...
            let delay_ms = now.duration_since(pkt.arrival_time).as_millis() as u32;
            ...
            let stretch_allowed = self.flow.stretch_allowed(now);
            ...
            self.tally.rms_sum += rms;
            self.tally.rms_count += 1;
            self.tally.rms_max = self.tally.rms_max.max(rms);
            ...
            if over_high && !stretch_allowed {
                self.tally.declined_recovery += 1;
            }
            ...
            if stretch_allowed && over_high {
                ...
                if is_fast || self.timescale_cooldown == 0 {
                    ...
                    self.timescale.window_begin(self.decoder.decoded());
                    if self.buffer.has_next()
                        && self.timescale.window_headroom() >= self.decoder.decode_len
                    {
                        let next_pkt = self.buffer.pop_next().expect("has_next was true");
                        self.decoder.capture(&next_pkt);
                        self.timescale.window_extend(self.decoder.decoded());
                        ...
                        self.tally.staged_pops += 1;
                    }
                    let window_frames = self.timescale.window().len() / OPUS_FRAME_SAMPLES;
                    let spliced = self
                        .timescale
                        .accelerate(is_fast, rms, &mut self.playback_buf);
                    if let Some(removed_samples) = spliced {
                        ...
                        self.tally.accelerated += 1;
                        if is_fast {
                            self.tally.fast_accelerated += 1;
                        }
                        self.tally.removed_frames += removed_frames;
                        tracing::trace!(
                        ...
                        if !masked {
                            self.tally.loud_splices += 1;
                        }
                        self.note_splice_quality(rms);
                    } else {
                        self.tally.declined_ncc += 1;
                        self.timescale.emit_window(&mut self.playback_buf);
                    }
                    return;
                }
                ...
                if self.timescale_cooldown > 0 {
                    self.tally.declined_cooldown += 1;
                }
                    ...
                    && self.config.static_target_ms.is_none()
                {
                    ...
                    if let Some(inserted_samples) = spliced {
                        ...
                        self.tally.expanded += 1;
                        if !imminent_underrun {
                            self.tally.preemptive += 1;
                        }
                        self.tally.inserted_frames += inserted_frames;
                    ...
                    if !imminent_underrun {
                        ...
                        self.tally.declined_underrun_ncc += 1;
                    }
        ...
        self.flow.missing_count += 1;
        self.observe_delivery(true, now);
    ...
    fn log_depth_authority(&mut self, breakdown: &TargetBreakdown, target: u32, now: Instant) {
        ...
        if self.log_frame_count < LOG_INTERVAL_FRAMES {
            return;
        }
        ...
        let window_ms = window.map_or(LOG_INTERVAL_FRAMES * MILLIS_PER_FRAME, |d| {
            d.as_millis() as u32
        });
    ...
    fn flush_with_crossfade(&mut self, flush_to: u32) {
        ...
        self.tally.flush_discards += self.buffer.occupied_count() - flush_to;
    ...
    fn generate_plc(&mut self) {
        ...
        self.tally.conceal_run_max = self.tally.conceal_run_max.max(self.flow.conceal_run);
        ...
        if self.decoder.plc_is_valid || !self.timescale.conceal_frame(&mut self.decoder.decode_buf)
        {
            self.decoder.decode_plc();
        } else {
            self.decoder.decode_len = OPUS_FRAME_SAMPLES;
            self.tally.pitch_conceals += 1;
            ...
            if let Some(step) = self.timescale.take_conceal_step() {
                self.tally.conceal_step_max = self.tally.conceal_step_max.max(step);
            }
        }
...
mod tests {
    ...
    fn rebuffering_at_target_after(
        ...
        manager.tally.flush_discards = 0;
    ...
    fn a_rebuffer_resume_should_not_clamp_below_the_measured_gap() {
        ...
        let (_, stale_high) = TargetController::buffer_limits(TARGET);
        let (live_low, live_high) = TargetController::buffer_limits(TARGET.max(raw_target));
        assert_eq!(
    ...
    fn a_rebuffer_resume_should_never_clamp_lower_than_the_ramped_band() {
        ...
        let unpause = ((TARGET as f32 * manager.config.resume_threshold_pct) as u32)
            .max(manager.min_depth_frames());
        let raw_only_band = TargetController::buffer_limits(raw_target).1;
        assert_eq!(
            manager.buffer.occupied_count(),
            high_limit - 1,
    ...
    fn a_target_step_up_should_converge_when_arrivals_match_playback() {
        ...
        let (low_limit, _) = TargetController::buffer_limits(manager.control.effective_target);
        assert!(
            low_limit > 1,
        ...
        for _ in 0..200 {
            ...
            let delta = manager.tally.inserted_frames - before;
            inserted += if delta >= 0.0 {
                delta
            } else {
                manager.tally.inserted_frames
            };
        }
    ...
    fn preemptive_expand_should_leave_at_most_one_pitch_period_of_surplus() {
        ...
        for _ in 0..300 {
            ...
            let before = manager.tally.expanded;
            manager.fill_output(&mut output, 1.0);
            if manager.tally.expanded > before {
                splices += 1;
    ...
    fn moderate_loud_overrun_should_be_drained_not_tolerated() {
        ...
        for _ in 0..200 {
            ...
            let (_, live_high) = TargetController::buffer_limits(manager.control.effective_target);
            ...
            let accel_before = manager.tally.accelerated;
            manager.fill_output(&mut output, 1.0);
            if manager.tally.accelerated > accel_before {
                accelerations += manager.tally.accelerated - accel_before;
            }
        }
        ...
        assert_eq!(
            manager.tally.declined_rms_mask, 0,
    ...
    fn a_declined_drain_should_report_why_it_was_declined() {
        ...
        // Fewer callbacks than LOG_INTERVAL_FRAMES, so the window this asserts on
    ...
    fn a_loud_splice_that_fails_the_ncc_gate_must_still_emit_its_window_verbatim() {
        ...
        ts.window_begin(&first);
        assert!(ts.window_extend(&second));
        let mut playback = VecDeque::new();
        let spliced = ts.accelerate(false, rms, &mut playback);
        assert!(
            spliced.is_none(),
        ...
        let staged = ts.window().len();
        ts.emit_window(&mut playback);
        assert_eq!(
            playback.len(),
            staged,
    ...
    fn a_small_target_should_reach_the_emergency_tier_before_a_quarter_second_of_latency() {
        for target in 3u32..=8 {
            let (_, high) = TargetController::buffer_limits(target);
    ...
    fn a_large_target_should_not_enter_the_emergency_tier_on_ordinary_overshoot() {
        ...
        let (_, small_high) = TargetController::buffer_limits(4);
        assert!(
    ...
    fn the_depth_line_should_report_the_content_rms_it_gated_on() {
        ...
        assert!(
            manager.tally.rms_count > 0,
            "the gate ran on every one of those callbacks, so the census must have \
             sampled it",
        );
        let avg = manager.tally.rms_sum / manager.tally.rms_count as f32;
        assert!(
    ...
    fn expand_should_defend_an_imminent_underrun_on_loud_content() {
        ...
        for _ in 0..200 {
            ...
            seq += 1;
            manager.ingest_packets(&mut cons);
            let expanded_before = manager.tally.expanded;
            manager.fill_output(&mut output, 1.0);
            if manager.tally.expanded > expanded_before {
                expands += manager.tally.expanded - expanded_before;
            }
            if manager.buffer.occupied_count() <= 1 {
                saw_trigger = true;
            }
    ...
    fn the_reported_rms_should_average_across_the_window_not_latch_the_last_frame() {
        ...
        let avg = manager.tally.rms_sum / manager.tally.rms_count.max(1) as f32;
        let peak = manager.tally.rms_max;
        assert!(
            peak > avg * 2.0,
        ...
        manager.tally = TimescaleTally::default();
        assert_eq!(manager.tally.rms_max, 0.0);
    ...
    /// `LOG_INTERVAL_FRAMES * MILLIS_PER_FRAME` long expects 100 packets and sees
    ...
    fn under_delivery_should_fire_when_arrivals_fall_below_the_expected_packet_rate() {
        ...
        let assumed = JitterBufferManager::expected_packets(LOG_INTERVAL_FRAMES * MILLIS_PER_FRAME);
        assert!(
    ...
    mod unplayed_frame_ledger {
        ...
        fn a_packet_arriving_behind_the_playhead_should_be_counted_as_a_stale_reject() {
            ...
            assert_eq!(manager.tally.stale_lag_sum, 4);
        }
        ...
        fn a_playhead_skip_should_report_how_many_frames_it_jumped() {
            ...
            assert!(
                manager.tally.playhead_skips >= 1,
            ...
            assert!(
                manager.tally.skipped_frames > manager.tally.playhead_skips,
        ...
        fn the_unplayed_ledger_should_close_when_the_drain_stages_a_frame() {
            ...
            // Fewer callbacks than `LOG_INTERVAL_FRAMES`: the log line takes the
            ...
            let occupied = manager.buffer.occupied_count();
            let played = manager.frames_played;
            let staged = manager.tally.staged_pops;
            ...
            assert_eq!(
                (
                    manager.tally.stale_rejects,
                    manager.tally.skipped_frames,
                    manager.tally.flush_discards,
                    manager.tally.shed,
                ),
                (0, 0, 0, 0),
        ...
        fn a_declined_underrun_should_no_longer_be_possible() {
            ...
            assert!(
                at_edge + manager.tally.expanded >= CALLBACKS,
            ...
            assert!(
                manager.tally.expanded <= CALLBACKS / MIN_EXPAND_INTERVAL + 1,
            ...
            for _ in 0..CALLBACKS {
                ...
                let (low_limit, _) =
                    TargetController::buffer_limits(manager.control.effective_target);
    ...
    mod conceal_run {
        ...
        fn the_depth_line_should_report_the_windows_longest_conceal_run() {
            ...
            assert_eq!(
                manager.tally.conceal_run_max, run,
            ...
            manager.ingest_packets(&mut cons);
            manager.log_frame_count = LOG_INTERVAL_FRAMES - 1;
            manager.fill_output(&mut output, 1.0);
            assert_eq!(
                manager.tally.conceal_run_max, 0,
    ...
    mod pitch_concealment {
        ...
        fn concealment_on_an_uncompressed_stream_should_not_emit_digital_silence() {
            ...
            assert_eq!(
                manager.tally.pitch_conceals, 1,
            ...
            assert!(
                manager.tally.conceal_step_max > 0.0,
        ...
        fn a_run_of_concealed_callbacks_should_stay_audible_until_the_fade_begins() {
            ...
            for expected in 1..=3u32 {
                ...
                assert_eq!(manager.tally.pitch_conceals, expected);
            }
        }

        /// **The Opus path must be bit-identical**, which is the property that makes
        /// this change safe to ship without a second field capture: `plc_is_valid`
        ...
        fn concealment_on_an_opus_stream_should_still_use_the_codec() {
            ...
            assert!(manager.flow.conceal_run > 0, "precondition: a standing run");
            assert_eq!(
                manager.tally.pitch_conceals, 0,
    ...
    mod conceal_fade_floor {
        ...
        fn the_concealment_fade_should_never_reach_digital_silence() {
            ...
            assert_eq!(
                manager.tally.pitch_conceals, 30,
    ...
    mod floor_frames {
        ...
        fn the_depth_line_should_count_frames_emitted_at_the_fade_floor() {
            ...
            for expected in 1..=8u32 {
                manager.fill_output(&mut output, 1.0);
                assert_eq!(
                    manager.tally.floor_frames, expected,
