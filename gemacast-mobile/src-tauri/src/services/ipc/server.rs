...
pub struct ServiceCommandListener {
    notifier: Arc<dyn FrontendNotifier>,
    cache_dir: Option<std::path::PathBuf>,
}

impl ServiceCommandListener {
    pub fn new(notifier: Arc<dyn FrontendNotifier>, cache_dir: Option<std::path::PathBuf>) -> Self {
        Self {
            notifier,
            cache_dir,
        }
    ...
    pub async fn run(self) {
        ...
        while let Ok((len, _)) = socket.recv_from(&mut buf).await {
            ...
            self.notifier.emit_service_command(command.to_string());
        }
    }
}
